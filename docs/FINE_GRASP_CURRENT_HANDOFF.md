...
随后 `codex/label-to-grasp` 按实测重新选用原近距离腕部安装位 `[0.060, 0, -0.030]`：
合并代码上完整抓起方块，而宽视野方案出现目标裁切；两套 profile 均保留，默认为近距离方案。
当前配置、倒置视野原因及合并后结果以 [标签抓取交接说明](LABEL_TO_GRASP.md) 为准。
...
上述 fallback 是历史实验配置：当前默认已禁用自动几何回退，模型不可用时返回失败。
