...
场景中的可移动物体也能作为放置支撑对象，例如“把红色方块放到黄色柱子上”。配置的 `objects` 与 `destinations` 仍区分物理资产类型，但不再限制谁可以作为支撑目标；能力接口返回全部场景实体作为放置候选。抓取对象与支撑对象必须是不同实体。是否能稳定放置由执行后的实际接触与稳定性判断，不要求方块底面完全包含在支撑面的轮廓内，也不使用配置中的初始位置评测已移动的支撑对象。
...
HTTP 202 仅表示接受任务。最终 `completed` 必须来自物理评测；失败或停止时返回 `failed / cancelled`。评测检查实际抬升、夹爪张开、末端退离、与指定支撑对象的向上接触力、物体位于支撑对象上方、释放后位移及两个物体的线速度。支撑对象的位置来自当前仿真状态；底面包围盒间距仅作诊断，不作为拦截条件。每次任务保存 JSON 及三路图像到 `output/arena/`。
...
- **当前分割已使用 YOLOE / SAM2 Tiny / SAM3，Florence 提供通用视觉任务。** 详细链路、图像与总线边界见 [视觉链路](ARENA_VISION_PIPELINE.md)。任务仍使用示例对象目录绑定 Arena 评测对象，尚不能宣称完成任意工业场景的目标关联。
