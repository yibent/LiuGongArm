...
2026-09-06 后续联调发现：抓放链路已有图像推理，但“查找目标”仍路由到旧 `/api/ground`，快速聊天也未读取 `status.vision`。当前已改为 Arena 的一次参数化 `perceive` 任务，识别进度经过原设备适配节点和视觉节点；聊天只读取最近一次目标观测的白名单文字字段，明确标注它不是全场景物体清单。旧观测不会挂到新命令上。
