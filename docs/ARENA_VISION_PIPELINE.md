...
Arena 在仿真线程冻结 RGB、深度、内参和外参，写入同机 `output/perception/<request_id>/`；后台线程仅向 `127.0.0.1:5570/observe` 发送观测编号。视觉模型读取 RGB，掩码保存到原目录，Arena 用同一帧的深度和位姿反投影。配置对象仅提供查询别名和任务绑定，不作为视觉检测答案；物体真值只用于物理任务与成功评测。Panda 的最终物理评测直接生成文字反馈，避免重复语言模型调用把放置失败改说成抓取失败，或附加旧机械臂的回位确认。
...
视觉查询支持未在任务目录登记的标签；当前抓放执行与评测仍绑定示例任务资产。运动继续快速 Panda 抓放优先，复杂或失败后 GraspGenX / AnyPlace 增强，Arena 执行 IK 与物理评测。VLA 未加载；任意移动目标的机械臂视觉伺服、悬挂、插入尚未接通。
