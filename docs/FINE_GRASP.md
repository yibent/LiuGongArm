...
抓取成功后，运行时把现有驱动目标交接给普通运动通道持续保持，而不是把夹爪目标改成实测开度。闭爪的目标角误差产生夹持力，因此清理阶段不再调用会卸载该误差的 `stop()`，也不恢复旧夹爪力矩限制。`GRASP_HOLD_HANDOFF` 记录交接目标；状态字段 `gripper_hold_active` 表示保留闭爪驱动目标，不等价于持续视觉确认仍持物。暂停、停止和后续手臂移动保留夹持；只有明确夹爪指令（或下一次抓取的开爪阶段）接管夹爪。移动到位判据排除正在持物而无法到达空载闭合角的夹爪，避免把正常夹持误判为运动超时。

保持修复回归：`waypoint-regression-grasp-aab8aa7d5648` 抓取成功后，观察 120 秒墙钟时间（仿真推进约 53 秒），闭爪目标保持 -2.865°，实测夹爪约 30.098°，末端和悬空方块保持稳定。每 30 秒保存状态及场景图；证据在远端 `/root/gpufree-data/deploy-backups/grasp-hold-26dnOC51/hold-soak.json` 与 `held-120s.jpg`。本地与云端 276 项 Python 回归测试通过。
