...
`inspect_object(kind=axis)` 返回长轴及两端的图像位置。后续已接通 `orientation={axis_ref,endpoint,direction}` 的执行与独立评测，横倒件转正入格组件首次通过；闭口端仍需视觉模型判断，PCA 不提供端面语义。进展与失败证据见 [Q8 后续闭环测试](Q8_CLOSED_LOOP_PROGRESS.md)。
