...
- 左侧上下文属性、对象位置/旋转、Panda 状态与执行参数、当前对话；上下/左右面板可拖动分隔线调整。
- 同页工作区、侧视、腕部三路实时预览；工作区占主画面，两路小画面等大；单路/三路、完整/半/四分之一采样、全屏。
- 四条时间轨道、拖动浏览、缩放、选择节点、因果端点配色、快绿慢蓝、运行中宽度增长。时间轴是只读记录，不修改执行顺序。
- 悬浮语音球、悬停或键盘聚焦后的文字入口、渐隐回复、当前对话查看。图片继续仅由预览与视觉服务处理，不进入语言模型上下文。
