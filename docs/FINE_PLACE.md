...
2026-09-05：全套 284 项通过，其中放置专项 42 项。日志在本地 `output/fine_place/unit_tests.log`。尚无成功放置验收记录；不能把下面的逐轮修复统计成成功率。
...
每段实际 FK 路径还受局部距离/旋转范围限制：不能只因终点距离 6 mm 就接受绕远的大 IK 分支。退出空间预检按“已张开”的夹爪几何进行，开爪扫掠则单独检查；不会拿夹持时闭合的手指体积模拟已经松开的退出动作。
