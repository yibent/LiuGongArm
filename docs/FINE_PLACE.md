...
- `31_fast_slow_payload`：快慢持物轮廓更新后的物理复测，结果待补充。
...
运动或释放检查仅因计算耗时导致画面过期时，最多重新采集并完整检查 3 次；任何一次碰撞、目标移动、持物滑移仍直接失败。始终拿不到时效合格的画面也失败，不修改时间戳、不扩大时效门限。释放前重新观察同时核验目的地和持物，开爪事件只在检查通过后产生。
