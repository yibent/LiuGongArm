...
`parent` 为目的地几何，`child` 为待放物体几何；两者同一坐标系、单位米。AnyPlace 推理服务将不足 1024 点的非空观测重复采样至固定输入长度，保留真实点数和重复采样数，不生成新的表面或宣称几何完整。旧版 `AnyPlaceBackend` 的执行前检查不属于当前 Arena 路径。已跳过关闭分类器时未使用的 8192 点采样；官方 stacking 固定种子前后 20 个输出变换逐位一致（`03_skip_unused_classifier.json`），不代表所有输入均已做等价验证。
