...
本次是 `codex/label-to-grasp` 的开发检查点，不是泛化验收通过或真机可直接部署的发布版。

- **本地视觉环境**：独立 vision overlay、固定版本的 Florence/YOLOE 权重下载与校验、离线启动；不改动 GraspGenX 模型环境。对应提交 `1262ee5`。
- **标签到抓取入口**：新增本地语义定位服务、同帧 RGB-D 米制定位、纯 LK 光流、分段粗接近与移动中停止检查；腕部最新观测通过身份/时间检查后，交给原 FineGrasp。对应提交 `2c6500a`、`8520105`。
- **主线兼容**：合并 `f9b0c65`，保留视觉记忆和 Web 双视角接口，解决与离线视觉配置、运行入口及执行器的冲突；不把主线的新相机安装位作为默认依据。合并提交 `3ed87b7`。
- **相机与诊断**：恢复原成功近距离安装位，宽视野仅显式对照；记录同帧手眼变换链、光轴和实际相机配置；修复粗接近扰动录像的错误标注。对应提交 `c9b249a`。
- **复现与回归**：提供下面的一条命令、242 项通过的单测，以及开发场景 01–16 的成功/失败记录；方块静态与 2.5 cm 移动后的完整抓取已实跑，杯子伺服失败和未完全达到抬升终点也如实保留。
- **接入边界**：`FineGraspRequest/Result` 上层接口不变；新的粗接近装配契约见“BusAgent 接口”。YOLOE 文本优先分支的额外依赖、工业工具/薄件/真实水果及真机验收仍未完成，见文末限制。

GitHub 交付包含代码、配置、环境/权重获取脚本、测试和工程文档。
`_models/`、`_envs/`、`_vendor/`、`output/` 不进 Git；录像和原始 RGB-D/日志保存在当前开发机的 `output/label_grasp/`，克隆仓库不会自动获得这些本地证据。
需要复现时按 [环境说明](LOCAL_VISION_SETUP.md) 准备依赖并重新运行，不能将文档中的历史结果视为新机器测试结果。
