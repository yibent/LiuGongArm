...
def _instance_association(payload, mask):
    """Keep pixel evidence when an annotator cannot resolve an instance."""
    result = dict(object_id=None, reason="missing_annotator", candidates={}, labels={})
    ...
    if ids.shape != mask.shape or not np.any(mask):
        result.update(reason="shape_mismatch_or_empty_mask", shape=list(ids.shape))
        return result
    ...
    for value, count in zip(*np.unique(ids[mask], return_counts=True)):
        ...
        if len(result["labels"]) < 8:
            result["labels"][str(int(value))] = dict(pixels=int(count), label=str(label)[:240])
        path = label if isinstance(label, str) else label.get("primPath") if isinstance(label, dict) else None
    ...
    result.update(candidates=counts, mask_pixels=int(mask.sum()), reason="no_instance_paths")
    if not counts:
        return result
    selected = max(counts, key=counts.get)
    coverage = counts[selected] / int(mask.sum())
    result.update(coverage=coverage, reason="insufficient_coverage")
    if coverage >= .6:
        result.update(object_id=selected, reason="matched")
    return result


def instance_association(payload, mask, leaf_payload=None):
    """Use leaf mesh paths too, without guessing from class or spawn position.

    Both annotators must refer to the same camera acquisition. Leaf meshes
    are aggregated under their prop root; conflicting confident IDs fail.
    """
    grouped = _instance_association(payload, mask)
    leaf = _instance_association(leaf_payload, mask)
    paths = {r["object_id"] for r in (grouped, leaf) if r["object_id"]}
    object_id = next(iter(paths)) if len(paths) == 1 else None
    reason = "matched" if object_id else "conflicting_instances" if len(paths) > 1 else "unresolved"
    return dict(object_id=object_id, reason=reason, grouped=grouped, leaf=leaf)


def instance_object_path(payload, mask):
    """Compatibility wrapper for a single confirmed segmentation payload."""
    return _instance_association(payload, mask)["object_id"]
    ...
    def resolve(self, control, request, timeout=8):
        ...
        try:
            ...
            if selected:
                ...
                result['association'] = selected.get('association')
